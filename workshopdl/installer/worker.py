...
class InstallWorker(QThread):
    ...
    def run(self):
        ...
        for i, (mod_id, folder) in enumerate(self.mod_folders.items(), 1):
            try:  # Подключаем ловец ошибок для текущего мода
            ...
            except Exception as e:  # Сюда попадут любые непредвиденные сбои в коде
                fail += 1
                self.log_line.emit(f"  💥 КРИТИЧЕСКАЯ ОШИБКА при обработке мода {mod_id}: {e}")
                
                # По желанию можно вывести подробный трейсбэк ошибки в лог:
                import traceback
