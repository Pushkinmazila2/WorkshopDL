        ...
        try:
            ...
            for i, (mod_id, folder) in enumerate(self.mod_folders.items(), 1):
                try:
                    ...
                    fail += 1
                    self.log_line.emit(f"  💥 Ошибка кода на моде {mod_id}: {mod_err}")
                    import traceback
                    self.log_line.emit(traceback.format_exc())
                    self.mod_status.emit(mod_id, False)

                self.progress.emit(total, total)
                self.finished.emit(success, fail)
        except Exception as global_thread_err:
            self.log_line.emit(f"  💥 Критическая ошибка потока: {global_thread_err}")
            import traceback  
            print("Критический сбой потока установки:", traceback.format_exc())
            # Безопасно закрываем поток, чтобы Qt не падал в c0000409
