"""
WorkshopDL — Python Edition v3 (PyQt6 Port)
...
from PyQt6.QtWidgets import (
    ...
    QHeaderView, QAbstractItemView, QSizePolicy, QToolBar, QSpinBox
)
from PyQt6.QtCore import Qt, QThread, pyqtSignal, QUrl
from PyQt6.QtGui import QFont, QColor, QBrush, QDesktopServices, QAction
...
class DownloadWorker(QThread):
    ...
    def _run_batch(self, mod_ids: list) -> dict:
        ...
        try:
            ...
            if IS_LINUX:
                custom_env["HOME"] = APP_DIR
            proc  = subprocess.Popen(
                args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                text=True, encoding="utf-8", errors="replace",
                creationflags=flags, env=custom_env
            )
            for line in proc.stdout:
                line = line.rstrip()
...
class MainWindow(QMainWindow):
    ...
    def _tab_updates(self):
        ...
        self.cmb_update_paths.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        ...
        self.upd_table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows)
        self.upd_table.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)
        self.upd_table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
    ...
    def _tab_settings(self):
        ...
        for lbl_key, attr, echo in [("settings_login", "inp_user", False),
            ...
            field = QLineEdit()
            if echo: field.setEchoMode(QLineEdit.EchoMode.Password)
        ...
        self.cmb_lang.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
    ...
    def _refresh_history(self):
        ...
        for gid, name in history_load().items():
            ...
            item = QListWidgetItem(label); item.setData(Qt.ItemDataRole.UserRole, gid)
            self.history_list.addItem(item)

    def _history_use(self):
        item = self.history_list.currentItem()
        if item: self.inp_game.setText(item.data(Qt.ItemDataRole.UserRole))
    ...
    def _history_delete(self):
        item = self.history_list.currentItem()
        if not item: return
        data = history_load(); data.pop(item.data(Qt.ItemDataRole.UserRole), None)
    ...
    def _on_finished(self, success, fail):
        ...
        game_id = self.inp_game.text().strip()
        if IS_LINUX:
            folder = os.path.join(APP_DIR, "Steam", "steamapps", "workshop", "content", game_id)
        else:
            folder = os.path.join(os.path.dirname(self._get_steamcmd()), "steamapps", "workshop", "content", game_id)
        self._scan_and_refresh_history()
    ...
    def _on_upd_result(self, mod_id, title, local_ts, server_ts, status, folder, size_mb, mod_missing):
        ...
        def cell(text, sort_val=None, align=Qt.AlignmentFlag.AlignCenter):
            it = QTableWidgetItem(text)
            it.setBackground(QBrush(row_color))
            it.setTextAlignment(align)
            if sort_val is not None: it.setData(Qt.ItemDataRole.UserRole, sort_val)
            return it
        ...
        tog_item.setData(int(Qt.ItemDataRole.UserRole.value) + 1, folder)
        # 5: папка
    ...
    def _on_upd_finished(self, outdated, ok_count):
        self.upd_table.setSortingEnabled(True)
        self.upd_table.sortByColumn(0, Qt.SortOrder.AscendingOrder)
    ...
    def _upd_table_clicked(self, row, col):
        if col == 3:   # Steam
            ...
            if tog:
                mod_id = tog.data(Qt.ItemDataRole.UserRole)
                folder = tog.data(int(Qt.ItemDataRole.UserRole.value) + 1)
                ...
                tog.setText(new_lbl)
                tog.setData(int(Qt.ItemDataRole.UserRole.value) + 1, new_folder)
        elif col == 5:  # Папка
            fold_item = self.upd_table.item(row, 4)  # папка хранится в col 4 UserRole+1
            if fold_item:
                folder = fold_item.data(int(Qt.ItemDataRole.UserRole.value) + 1)
